...
const epng::rgba_pixel TRAILER_COLOR = color::BLUE;

const int CABIN_WIDTH = 8;
const int CABIN_HEIGHT = 12;
const epng::rgba_pixel CABIN_COLOR = color::RED;

const int ENGINE_WIDTH = 6;
const int ENGINE_HEIGHT = 8;
const epng::rgba_pixel ENGINE_COLOR = color::RED;

const int WINDOW_WIDTH = 4;
const int WINDOW_HEIGHT = 4;
const epng::rgba_pixel WINDOW_COLOR = color::BLUE;

const int WHEEL_RADIUS = 3;
const epng::rgba_pixel WHEEL_COLOR = color::BLACK;
...
Truck::Truck(const Vector2& pcenter)
    : center_{pcenter},
      ...
      trailer{new Rectangle{Vector2{center_.x() - TRAILER_WIDTH / 2,
                                    center_.y()},
      ...
      window{new Rectangle{Vector2{cabin->center().x(),
                                   cabin->center().y() - WINDOW_HEIGHT / 2},
...
void Truck::draw(epng::png& canvas)const
{
trailer->draw(canvas);
cabin->draw(canvas);
window->draw(canvas);
