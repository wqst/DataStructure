...
/**
 * Pruint64_ts a direction to a stream.
 * @param os the stream to pruint64_t to
 * @param dir the direction to pruint64_t
...
class maze
{
  public:
     ...
     *
     * Huint64_t: You should use an instance of a graph object from mp_graph.2
     ...
     * present.  Our tests will call set_wall to copy a specific maze uint64_to
     * your implementation.
     *
     ...
     *
     * Huint64_t: this function should run in time linear in the number of cells
     * in the maze.
     *
  ...
  private:
    ...
    dsets cellset;
    vector< pair< pair<uint64_t, uint64_t>, direction > >  walls;
    vector<bool> rw;
    vector<bool> bw;

    uint64_t idx(uint64_t x, uint64_t y) const;
    uint64_t xcoord( uint64_t location) const;
    uint64_t ycoord( uint64_t location) const;
    uint64_t next( uint64_t x, uint64_t y, direction dir );
    void makecolor(uint64_t xc, uint64_t yc, epng::png& retval);

    /**
     * Our recommended storage for the maze. This is *not* the *only* way
     * to do it, it's just a particularly nice way.
     *
     * As a huint64_t: remember that the vertex ids in the graph are uint64_tegers 0
     * through \f$n\f$ where \f$n\f$ is the number of times you have called
     * `add_vertex()`.
     */
    //graph<edge_weights<uint64_t>> graph_;
};
}
#endif
